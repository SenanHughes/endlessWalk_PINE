	...
	: Super(ObjectInitializer)
{
	...
	MeshGeneration = CreateDefaultSubobject<UProcMeshGeneration_CPP>(TEXT("MeshGeneration"));
	//MeshGeneration->RegisterComponent();

	MeshUpdates = CreateDefaultSubobject<UProcMeshUpdates_CPP>(TEXT("MeshUpdates"));
	//MeshUpdates->RegisterComponent();

	SpawnAssets = CreateDefaultSubobject<USpawnAssets_CPP>(TEXT("SpawnAssets"));
	//SpawnAssets->RegisterComponent();

	UpdateAssets = CreateDefaultSubobject<UUpdateAssets_CPP>(TEXT("UpdateAssets"));
...
void AProceduralLevelGenerator_CPP::BeginPlay()
{
	...
	SpawnAssets->SpawnAssetInstances(PlantDynamicData.PlantInstance, MoundDynamicData.MoundSpline, MoundDynamicData.MoundMesh, 1, 3, PlantDynamicData.ValidSpawnPoints, PlantDynamicData.InstanceCount);
	SpawnAssets->SpawnAssetInstances(PlantDynamicData1.PlantInstance, MoundDynamicData.MoundSpline, MoundDynamicData.MoundMesh, 2, 4, PlantDynamicData1.ValidSpawnPoints, PlantDynamicData1.InstanceCount);
...
void AProceduralLevelGenerator_CPP::Tick(float DeltaTime)
{
	...
	if (FVector(CharacterLocation - MidPoint).Size2D() <= PathConfigData.PathWidth / 2)
	{
		...
		UpdateAssets->UpdateAssetInstances(PlantDynamicData.PlantInstance, PathDynamicData.PathSpline, PathDynamicData.PathMesh, 1, 3, PlantDynamicData.ValidSpawnPoints, PlantDynamicData.InstanceCount, PlantDynamicData.LogicalStart);
		...
		UpdateAssets->UpdateAssetInstances(PlantDynamicData1.PlantInstance, MoundDynamicData.MoundSpline, MoundDynamicData.MoundMesh, 2, 4, PlantDynamicData1.ValidSpawnPoints, PlantDynamicData1.InstanceCount, PlantDynamicData1.LogicalStart);
...
void AProceduralLevelGenerator_CPP::UpdateTerrainSpline()
{
	...
	const float MinForwardAlignment = 0.8f;
